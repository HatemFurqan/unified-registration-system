...
class RegularStudentsController extends Controller
{
    public function index()
    {
        ...
        return view('regular.index', ['countries' => $countries, 'course' => $course]);
    }
    ...
    public function thankYouPage()
    {
        ...
        if (! (session('error') || session('success')) ) {
            return redirect()->route('registration.regular.index');
        }

        return view('regular.thank-you', ['countries' => $countries, 'course' => $course]);
    }
    ...
    public function resubscribe(Request $request)
    {
        ...
        if ($request->payment_method == 'checkout_gateway') {
            ...
            if ($amount > 0){
                try {
                    ...
                    return redirect()->route('registration.regular.thankYouPage');
                }
            }
            ...
            if ($amount > 0){
                ...
                if ($redirection){
                    ...
                    return view('regular.thank-you', ['countries' => [], 'course' => null]);
                }
            }else{
                session()->flash('success', __('resubscribe.The registration process has been completed successfully'));
                return redirect()->route('registration.regular.thankYouPage')->with(['subscribe_id' => $subscribe->id]);
            }
        }
        elseif ($request->payment_method === 'paypal') {
            ...
            Subscribe::createNewSubscribe($request, $student, $coupon, $have_discount, $discount * 100, $referenceNumber, 'paypal', $paypalTransaction['id'], 'Pending', FormType::REGULAR);
            return response()->json($paypalTransaction);
        }
        elseif ($request->payment_method === 'hsbc') {
            $referenceNumber = Subscribe::generateReferenceNumber();
            $subscribe = Subscribe::createNewSubscribe($request, $student, $coupon, $have_discount, $discount * 100, $referenceNumber, 'hsbc', null, 'Pending', FormType::REGULAR);
